      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="max-w-4xl mx-auto mb-10 space-y-5">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-12 pr-4 py-3.5 rounded-2xl bg-white border border-stone-300 shadow-xs focus:outline-none focus:border-[#C89D4B] focus:ring-2 focus:ring-[#C89D4B]/20 text-sm sm:text-base text-stone-900 transition-all placeholder:text-stone-500 font-medium"
            />
            {searchQuery && (
              <button
                onClick={() => setSearchQuery('')}
                className="absolute right-4 top-1/2 -translate-y-1/2 text-xs font-semibold text-stone-600 hover:text-stone-900 bg-stone-100 px-2.5 py-1 rounded-md"
              >
                Clear
              </button>
            )}
        ...
        {filteredItems.length === 0 ? (
          <div className="text-center py-16 bg-white rounded-2xl border border-stone-200 p-8 max-w-xl mx-auto">
            ...
            <p className="text-stone-800 text-base font-semibold mb-2">{t.menuPage.noResults}</p>
            <button
              onClick={() => { setSelectedCategory('all'); setSearchQuery(''); }}
              className="text-sm text-[#78350F] font-bold underline"
            >
              Reset Filters
            </button>
          </div>
            ...
            {filteredItems.map((item) => (
              ...
              >
                ...
                <div className="relative h-48 sm:h-52 overflow-hidden bg-stone-900">
                  ...
                  {item.isSignature && (
                    <div className="absolute top-3 right-3 flex items-center gap-1 px-2.5 py-1 rounded-md bg-[#2A1710]/95 text-[#DFBE7A] text-xs font-bold border border-[#DFBE7A]/40">
                ...
                <div className="p-5 flex-1 flex flex-col justify-between text-left">
                  <div>
                    <h3 className="font-serif font-bold text-xl sm:text-2xl text-[#2A1710] mb-1 group-hover:text-[#8C5E1E] transition-colors">
                      {language === 'ta' ? item.nameTa : item.name}
                    </h3>
                    
                    {language !== 'ta' && (
                      <p className="text-sm font-tamil text-[#78350F] font-semibold mb-2">
                        {item.nameTa}
                      </p>
                    )}
                  ...
                  <div className="mt-4 pt-3 border-t border-stone-100 flex items-center justify-between text-xs sm:text-sm text-stone-600">
                    <span className="text-emerald-700 font-semibold flex items-center gap-1">
                      🌿 100% Vegetarian
                    </span>
                    <span className="text-stone-500 font-medium">Fresh Preparation</span>
                  </div>
                </div>
              </div>
            ))}
        ...
        <div className="mt-12 p-5 rounded-xl bg-amber-50/90 border border-amber-300 max-w-3xl mx-auto text-left text-sm text-amber-950 flex items-start gap-3">
          <AlertCircle className="w-5 h-5 text-amber-800 shrink-0 mt-0.5" />
          <div>
            <p className="font-bold text-stone-950">{t.menuPage.priceDisclaimer}</p>
        ...
        <div className="mt-12 text-center flex flex-wrap items-center justify-center gap-4">
          <a
            ...
            className="inline-flex items-center gap-2 px-7 py-3.5 rounded-full bg-[#2A1710] text-[#DFBE7A] hover:bg-[#3D2318] hover:text-white text-sm sm:text-base font-bold shadow-md transition-all"
          >
          ...
          <a
            ...
            className="inline-flex items-center gap-2 px-7 py-3.5 rounded-full bg-emerald-700 text-white hover:bg-emerald-800 text-sm sm:text-base font-semibold shadow-md transition-all"
          >
